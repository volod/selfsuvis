...
#   Step 4 — Download / generate sensor sample data (Steps 9–19 sidecars)
#   Step 5 — Start Docker stack (Qdrant + PostgreSQL) and run DB migration
#   Step 6 — Locate or prompt for a test video
...
if $SENSOR_DATA_ONLY; then
  ...
  log "Copy the generated sidecars next to your video:"
  log "  cp data_test/sensors/step16_imu/sample_mission_042.imu.jsonl data_test/videos/mission.imu.jsonl"
  exit 0
fi
...
# =============================================================================
# STEP 4: SENSOR SAMPLE DATA (Steps 9–19)
#
# prepare_sensor_data.sh creates data_test/sensors/ with one subdirectory per
# sensor step, each containing:
#   - Downloaded public dataset samples (where freely available without login)
#   - Synthetically generated sidecar JSONL files (IMU, barometer, wind, env,
#     gas, radiation, ADS-B, acoustic events)
#   - README.txt per directory with licence notes and sidecar naming convention
#
# Steps requiring manual download (free but need registration):
...
#
# After this step, copy sidecars next to your test video:
#   cp data_test/sensors/step16_imu/sample_mission_042.imu.jsonl \
#      data_test/videos/mission.imu.jsonl
# =============================================================================
section "Step 4 — Sensor sample data (Steps 9–19)"

log "Downloading / generating sensor sample data..."
bash scripts/prepare_sensor_data.sh data_test/sensors
log "Sensor data in data_test/sensors/"
...
if $NO_DOCKER; then
  ...
  if ! command -v docker >/dev/null 2>&1; then
    ...
    make up 2>/dev/null \
      || warn "make up failed — check docker/docker-compose.yml and run 'make docker-check'."

    # Wait for PostgreSQL to be ready before running the migration.
    # The container typically takes 3–8 seconds to accept connections.
    log "Waiting for PostgreSQL to be ready (5 s)..."
    sleep 5

    log "Running database migration (creates all tables on first run)..."
    "$PYTHON" scripts/migrate_postgres.py \
      || warn "Migration failed — postgres may need more time. Retry: python scripts/migrate_postgres.py"
  fi
fi
...
# STEP 6: TEST VIDEO
#
# The pipeline requires at least one video file in data_test/videos/.
# Two sample clips are committed in the repo.  Add your own outdoor footage:
#
#   cp /path/to/drone_mission.mp4 data_test/videos/
#
# Free outdoor footage:
#   Mixkit:  https://mixkit.co/free-stock-video/nature/
#   Pexels:  https://www.pexels.com/search/videos/outdoor/
#   NASA:    https://images.nasa.gov/
#
# For sensor steps to work, place sidecar files with matching basename:
#   data_test/videos/mission.mp4          ← video
#   data_test/videos/mission.imu.jsonl    ← IMU sidecar (Step 16)
#   data_test/videos/mission.env.jsonl    ← atmospheric sidecar (Step 17)
#   … (see README sidecar naming convention)
#
# The sample sidecars generated in Step 4 use basename 'sample_mission_042'.
# Rename them to match your video, or use the provided examples as-is.
# =============================================================================
section "Step 6 — Test video"

mkdir -p data_test/videos

if ls data_test/videos/*.mp4 data_test/videos/*.mov 2>/dev/null | head -1 | grep -q .; then
  TEST_VIDEO="$(ls data_test/videos/*.mp4 data_test/videos/*.mov 2>/dev/null | head -1)"
  log "Test video found: $TEST_VIDEO"
else
  warn "No test video found in data_test/videos/"
  warn "Add a .mp4 or .mov file:"
  warn "  cp /path/to/your/mission.mp4 data_test/videos/"
  TEST_VIDEO="data_test/videos/mission.mp4"
fi
...
echo -e "${BOLD}2. Full run — Ollama sidecars + all sensor steps:${RESET}"
echo "   SENSOR_FUSION_ENABLED=true \\"
echo "   RF_ENABLED=true \\"
echo "   THERMAL_ENABLED=true \\"
echo "   LIDAR_ENABLED=true \\"
echo "   GAS_ENABLED=true \\"
echo "   ACOUSTIC_ENABLED=true \\"
echo "   .venv/bin/python main.py --mode local \\"
echo "     --input $TEST_VIDEO \\"
