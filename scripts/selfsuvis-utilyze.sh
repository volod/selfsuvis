...
Examples:
  ...
  ./scripts/selfsuvis-utilyze.sh --devices 0
  ./scripts/selfsuvis-utilyze.sh --endpoints
  UTLZ_DISABLE_METRICS=0 ./scripts/selfsuvis-utilyze.sh
EOF
  exit 0
fi

if ! command -v utlz >/dev/null 2>&1; then
  echo "utlz is not installed. Run ./scripts/install_utilyze.sh first." >&2
  exit 1
fi
