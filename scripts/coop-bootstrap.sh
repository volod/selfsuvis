...
If no arguments are passed, this runs: `./scripts/coop-compose.sh up -d`
...
if [[ ! -f "$(project_env_file)" ]]; then
  ...
  "$PROJECT_ROOT_DIR/scripts/coop-env.sh" "$ENV"
fi

# Ensure bind-mount directories exist
"$PROJECT_ROOT_DIR/scripts/coop-data-dirs.sh"
...
if [[ ! -f "$PROJECT_ROOT_DIR/config/coop/mosquitto/certs/server.key" ]]; then
  ...
  "$PROJECT_ROOT_DIR/scripts/coop-mosquitto-tls.sh" "$HOST"
fi
...
if [[ ! -f "$PROJECT_ROOT_DIR/config/coop/mosquitto/pwfile" ]]; then
  ...
  "$PROJECT_ROOT_DIR/scripts/coop-mqtt-users.sh"
fi

# Start the stack
if [[ $# -eq 0 ]]; then
  exec "$PROJECT_ROOT_DIR/scripts/coop-compose.sh" up -d
else
  exec "$PROJECT_ROOT_DIR/scripts/coop-compose.sh" "$@"
fi
