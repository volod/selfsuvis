...
def test_extract_frames_uses_timeout(tmp_path, monkeypatch):
    ...
    frames_dir.mkdir()
    monkeypatch.setattr("pipeline.ffmpeg_utils.settings", MagicMock(
    ...
    mock_run = MagicMock()
    with patch("pipeline.ffmpeg_utils.subprocess.run", mock_run):
        with patch("pipeline.ffmpeg_utils.os.listdir", return_value=[]):
