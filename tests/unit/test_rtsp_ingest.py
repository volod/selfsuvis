...
def test_dns_failure_raises():
    import socket
    with patch("pipeline.rtsp_ingest.socket.getaddrinfo", side_effect=socket.gaierror("NXDOMAIN")):
...
@patch("pipeline.rtsp_ingest.subprocess.run", side_effect=subprocess.CalledProcessError(1, "ffmpeg"))
