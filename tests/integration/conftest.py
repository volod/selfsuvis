...
No live Docker, GPU, or external service required.
External services (PostgreSQL, Qdrant, models) are stubbed in-process.
"""
...
class PipelineMockConn:
    """Simulates an asyncpg Connection for the full pipeline.

    Handles: jobs, missions, frames, scene_timeline, processed_files.
    All SQL is matched by keyword patterns that reflect the exact queries
    in pipeline/job_db_pg.py, pipeline/mission_db.py, and pipeline/rtsp_captioner.py.
    """

    def __init__(self):
        self._jobs: Dict[str, _Row] = {}
        self._missions: Dict[str, _Row] = {}
        self._frames: Dict[str, _Row] = {}           # keyed by frame id
        self._timeline: List[_Row] = []
        self._processed: Dict[str, _Row] = {}        # keyed by file_hash
    ...
    async def execute(self, query: str, *args) -> str:
        ...
        # ── Missions ──────────────────────────────────────────────────────────
            ...
            # mark_mission_finished / apply_gps_registration
        ...
        # ── Frames ────────────────────────────────────────────────────────────
            ...
            # apply_gps_registration frame poses — handled by executemany
            pass
    ...
    async def executemany(self, query: str, rows) -> None:
        ...
        elif "UPDATE FRAMES SET GLOBAL_POSE_JSON" in q:
    ...
    async def fetch(self, query: str, *args) -> List[_Row]:
        ...
        if "FROM FRAMES" in q and "ORDER BY CREATED_AT ASC" in q:
            ...
            sorted_frames = sorted(
                self._frames.values(),
                key=lambda f: (f.get("created_at") or datetime.min, f["id"])
            )
            ...
            if "(CREATED_AT, ID) >" in q:
                ...
                sorted_frames = [
                    ...
                    if (f.get("created_at") or datetime.min, f["id"]) > (cursor_ts, cursor_id)
                ]
        ...
        if "FROM SCENE_TIMELINE" in q:
            ...
            nearby = [
                ...
                if (t.get("gps_lat") is not None
                    and min_lat <= t["gps_lat"] <= max_lat
                    and min_lon <= t["gps_lon"] <= max_lon)
            ]
            nearby.sort(key=lambda r: r.get("created_at") or datetime.min, reverse=True)
            return nearby[:limit]

        return []

    # ── Context-manager wrappers (asyncpg pool.acquire() pattern) ─────────────
...
class PipelineMockPool:
    """Wraps PipelineMockConn so it supports `async with pool.acquire() as conn`."""
    ...
    def acquire(self):
        pool = self
        class _Ctx:
            async def __aenter__(self_inner): return pool.conn
            async def __aexit__(self_inner, *_): pass
        return _Ctx()

    # Direct coroutines (pool.execute / pool.fetch used by some helpers)
