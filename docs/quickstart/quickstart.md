...
| **Local Development** | Hot-reload development, working on pipeline code, or running the local learning pipeline (`ssv --mode local`) | [Quick Start — Local](quickstart-local.md) |
| **IoT / sencoop monitoring** | Add LoRaWAN sensors + Frigate cameras for live multi-modal site awareness and LLM scene synthesis | [sencoop — Getting Started](../sencoop/getting-started.md) |
...
1. Run the local video pipeline (`ssv --mode local`) for the core learning path.
...
- [MediaMTX streaming](../reference/streaming-mediamtx.md) — live RTSP/RTMP ingress, path control, and realtime stream analysis
...
selfsuvis ships a built-in IoT edge layer (`sencoop`) for monitoring physical
...
See [sencoop — Integration Guide](../sencoop/integration.md) for the full picture.
