...
Two paths: **Docker** (recommended, no host Python needed) and **local dev** (for development or when you want hot-reload).
...
Use this when you want hot-reload on the API or are working on the pipeline code directly.
...
Once all services are up (Docker or local dev), the indexing pipeline runs automatically in the background worker whenever a job is enqueued. Here are the three ways to trigger it.
...
| `<video>.gas.jsonl` | Step 18 — gas / radiation (CO₂, VOC, PM2.5, dose rate) |
