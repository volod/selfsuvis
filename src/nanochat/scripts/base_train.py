...
from nanochat.model.flash_attention import HAS_FA2, USE_FA2, USE_FA3
from nanochat.model.gpt import GPT, GPTConfig, Linear
from nanochat.tokenizer import get_token_bytes, get_tokenizer
from nanochat.training.checkpoint_manager import load_checkpoint, save_checkpoint
from nanochat.training.report import get_report
from nanochat.training.schedule import eval_step_count, should_eval_base
...
parser.add_argument(
    "--tokenizer-threads", type=int, default=0, help="tokenizer worker threads (0 = adaptive)"
)
parser.add_argument(
    "--tokenizer-batch-size", type=int, default=128, help="documents per tokenizer batch"
)
parser.add_argument(
    "--loader-buffer-size",
    type=int,
    default=0,
    help="pre-tokenized document queue depth (0 = adaptive)",
)
# Evaluation
parser.add_argument(
...
if USE_FA3:
    print0("[OK] Using Flash Attention 3 (Hopper GPU detected), efficient kernel active.")
elif USE_FA2:
    print0(
        "[OK] Using Flash Attention 2 (flash-attn installed), efficient kernel with sliding-window support."
    )
else:
    print0("!" * 80)
    if not HAS_FA2:
        print0(
            "WARNING: flash-attn not installed. Run 'make install-fa' for a ~2x speedup on Ada/Ampere GPUs."
        )
    else:
        print0(
            f"WARNING: Flash attention not active (COMPUTE_DTYPE={COMPUTE_DTYPE}, bf16 required)."
        )
...
if args.fp8:
    if device_type != "cuda":
        ...
        print0(
            f"[OK] FP8 training enabled ({args.fp8_recipe} scaling) - converted {num_fp8}/{num_linear} linear layers, skipped {num_skipped} (too small)"
        )
...
print0(
    "Compiling model with torch.compile (first forward pass triggers CUDA kernel compilation, ~2-5 min)..."
)
model = torch.compile(
    model, dynamic=False
...
print0(
    f"Data loader: tokenizer_threads={tokenizer_threads}, tokenizer_batch_size={args.tokenizer_batch_size}, buffer_size={loader_buffer_size}"
)
...
# The validation set is fixed for the whole run, but build_val_loader() used to be
# called on every eval (every eval_every steps), each time spawning a fresh tokenizer
# thread and re-tokenizing the val shard from scratch. We tokenize once and cache the
# batches on CPU (pinned for fast async HtoD), which also keeps val_bpb deterministic
# across evals so the curves are directly comparable.
_val_batches_cache: "list | None" = None


def get_val_batches(eval_steps):
    """Return an iterator of (inputs, targets) device tensors for `eval_steps` batches."""
    global _val_batches_cache
    if _val_batches_cache is None or len(_val_batches_cache) < eval_steps:
        loader = build_val_loader()
        cache = []
        for _ in range(eval_steps):
            xb, yb = next(loader)
            xb, yb = xb.to("cpu"), yb.to("cpu")  # clone off the loader's shared buffer
            if device_type == "cuda":
                xb, yb = xb.pin_memory(), yb.pin_memory()
            cache.append((xb, yb))
        _val_batches_cache = cache

    def _iter():
        non_blocking = device_type == "cuda"
        for xb, yb in _val_batches_cache[:eval_steps]:
            yield xb.to(device, non_blocking=non_blocking), yb.to(device, non_blocking=non_blocking)
...
while True:
    ...
    should_eval = should_eval_base(step, args.eval_every, args.eval_at_start, last_step)
    if should_eval:
        model.eval()
        ...
        val_batches = get_val_batches(eval_steps)
        with disable_fp8(model):
            val_bpb = evaluate_bpb(model, val_batches, eval_steps, token_bytes)
    ...
    # Running on-device sum of detached micro-batch losses. NaN in any micro-batch
    # propagates into the sum, so we can check once per step (below) instead of forcing
    # a CPU-GPU sync on every micro-step (which stalled the prefetch/backward pipeline).
    accum_loss = None
    for micro_step in range(grad_accum_steps):
        if _micro_print and micro_step % max(1, grad_accum_steps // 8) == 0:
            elapsed_ms = (time.time() - t0) * 1000
            _sys.stdout.write(
                f"\r  step {step:05d} | micro-batch {micro_step + 1:>3}/{grad_accum_steps} | {elapsed_ms:6.0f}ms"
            )
            _sys.stdout.flush()
        loss = model(x, y)
...
if _stop_requested:
    ...
    print0(
        f"Smooth train loss: {_smooth:.6f}"
        + (f"  |  val bpb: {val_bpb:.6f}" if val_bpb is not None else "")
    )
    if step > 0:
