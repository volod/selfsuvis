...
def _download_hf_dataset(cache_dir: Path) -> bool:
    ...
    try:
        ...
        _log.warning(
            ...
            "Run: pip install datasets  then  scripts/split_drone_audio_data.sh",
            _HF_REPO,
        )
        return False
...
def _write_report(
    ...
    lines += [
        ...
        "scripts/split_drone_audio_data.sh",
        "",
...
def step_drone_audio_training(
    ...
    if not train_items:
        _log.warning(
            ...
            "Run scripts/split_drone_audio_data.sh to prepare the dataset.",
            cache_dir,
        )
        result["skipped"] = True
